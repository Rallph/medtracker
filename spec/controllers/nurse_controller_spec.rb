...
RSpec.describe NurseController, type: :controller do
  ...
  describe 'Submit Administer Medication Form' do
    it 'should redirect to the administer form with a flash notice' do
      ...
      fake_students = double('student1', full_name: 'student1', id: 1) #, double('student2', full_name: 'student2', id: 2)]
      fake_approvals = [double('ap1', student_id: 1, student_medication_id: 1, school_medication_id: 1 ), double('ap1', student_id: 1, student_medication_id: 1, school_medication_id: 2)]
      expect(Student).to receive(:find).and_return(fake_students)
      expect(fake_students).to receive(:medication_approvals).with(no_args).and_return(fake_approvals)
    ...
    it 'should redirect to the administer form with a bad flash notice' do

      fake_students = double('student1', full_name: 'student1', id: 1) #, double('student2', full_name: 'student2', id: 2)]
      fake_approvals = [double('ap1', student_id: 1, student_medication_id: 2, school_medication_id: 2 ), double('ap1', student_id: 1, student_medication_id: 2, school_medication_id: 2)]
      expect(Student).to receive(:find).and_return(fake_students)
      expect(fake_students).to receive(:medication_approvals).with(no_args).and_return(fake_approvals)
      allow(SchoolMedicationTransaction).to receive(:create!).and_return(double('transaction', :decrease_medication_quantity => nil))
      post :administer_submit, { select_student: "1", select_school_medication: "1", dosage: "1", time: "2021-11-16T18:06", comment: ""}
